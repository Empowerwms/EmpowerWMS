...
class DeliveryBoyPickings(models.Model):
        ...
        for db_picking in self:
            if vals.get('picking_state') == 'denied' or vals.get('picking_state') == 'cancel':
                ...
                if delivery_boy_config.get('verify_token'):
                    ...
                    check_mail = delivery_boy_config.get('delivery_token_mail_temp_id').send_mail(self.id, email_values=email_values, force_send=True)
    ...
    def create(self, vals):
        ...
        rec.message_subscribe(partner_ids=user)
        return data
